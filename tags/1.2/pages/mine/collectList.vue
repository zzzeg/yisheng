<template>
    <view>
        ...
        <view class="content">
            <view v-if="current === 0">
                <view v-if="serverList && serverList.length > 0" style="margin:30rpx">
                	<block v-for="(item, index) in serverList" :key="index" >
                		<h-station-list :item='item'  @tap="clickServer(index)" @touchstart='touchstart' @touchend='touchend' @longtap="longtapServer(item,index)" ></h-station-list>
                	</block>
                </view>
            ...
            <view v-if="current === 2">
                <view class="living" v-if="videoList && videoList.length > 0">
                	...
                	<view class="box" v-for="(item, index) in videoList" :key="index">
                		<view class="disinline" style="font-weight: 500;font-size: 40upx;" @tap="goInfo(item.id)"  @touchstart='touchstart' @touchend='touchend' @longtap="longtapServer(item,index)">
                			{{ item.title }}
                		</view>
                		<view class="videos">
                			<view>
            ...
            <view v-if="current === 3">
                <view class="productBox" v-for="(item, index) in productList" :key="index" @click="clickProduct(item.productId,'community',item.communityId)"   @touchstart='touchstart' @touchend='touchend' @longtap="longtapServer(item,index)">
                    ...
                    <view style="margin:0 15upx; flex: 1;justify-content: center;display: flex;flex-direction: column;">
                        <view style="font-size: 33upx; color: #16202E;overflow: hidden; text-overflow: ellipsis; word-break: break-all;display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 2;">
...
<script>
    ...
    export default{
    	...
    	methods: {
            ...
            clickMarkBtn:function(){
                ...
                if(this.current === 3){
                    this.$api.productMarkDelete({
                    	productId:this.selectItem.productId
                    }).then(res=>{
                    	if(res.status=="OK"){
            ...
            goDetail({id,type,communityId}){
                if(this.touchE-this.touchT<350){
                    if(type=='VIDEO'){
                        uni.navigateTo({
                            url: `/pages/housekeeper-sharing-video/housekeeper-sharing-video?id=${id}&communityId=${communityId}`,
                        });
                    }else{
                        uni.navigateTo({
                            url: `/pages/housekeeper-sharing-img/housekeeper-sharing-img?id=${id}&communityId=${communityId}`,
                        });
                    }
                }
            },
    ...
    .living {
    	...
    	overflow: hidden;
    		...
    		.videos{ position: relative; z-index:1;  background:#ddd; margin:20rpx 0;font-size:24rpx; min-height:360rpx;
			 ...
			 button.zanico{ width:33rpx; height:33rpx; margin-left:26rpx; vertical-align: middle;padding: 0; background: url('../../static/image/share@2x.png') no-repeat center / 26rpx 26rpx;}
			 .zanico{ width:33rpx; height:33rpx; margin-left:26rpx; vertical-align: middle; }
    	}
    }
    .btnBox{
        display: flex;
