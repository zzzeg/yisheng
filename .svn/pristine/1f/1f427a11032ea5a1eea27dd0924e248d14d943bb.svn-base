	...
	<view style="padding-bottom:130rpx ;overflow: hidden;">
		...
		<template v-else>
			<view class="u-f-ac u-column" style="margin:100rpx 0 40rpx 0;">
				...
				<text class="noreport">您还没有上传任何体检报告</text>
				<text class="please">请点击下方的上传报告按钮上传</text>
			</view>
		</template>
		<view class="u-f-ajc done" @tap="gotoupload()">
	...
	export default {
		...
		methods: {
			...
			gotoupload(){
				console.log('uppp');
				uni.navigateTo({
					url:'/pages/upload-report/upload-report?type=checkupReport'
				})
			},
			gotoDetail(id){
				uni.navigateTo({
					url:'/pages/look-report/look-report?type=checkupReport&id='+id
				})
			},
			deleteItem(id){
				this.currentId = id
			...
			editReport(id){
				uni.navigateTo({
					url:'/pages/upload-report/upload-report?type=checkupReport&id='+id
				})
			}
		},
		onReachBottom() {
