		...
		<view class="health-top-content">
			<view class="address">
				<view style="display: inline-block; vertical-align: middle; text-align: center;font-size: 15px;margin-left: 32rpx;"  @tap="showall(true)">
					<view class="disinline" style="max-width:4em; overflow: hidden;white-space: nowrap; text-overflow: ellipsis;">{{ addrCheck.length>0  ? addrCheck[0] : '选择城市'}}</view>
		...
		methods: {
							...
							if(q.name == province){
								q.children.map(i=> {
									if(i.name==that.addrCheck[0]) {
                                        that.areaChidren = that.areaChidren.concat(i.children);
									}
								})
							}
						})
			...
			checkItem(pitem, item, itemall) {
				...
				if(itemall.children.length > 0) {
						this.areaChidren = this.areaChidren.concat(itemall.children)
				}
			    this.page = 1
				this.getHospitalList(true)
