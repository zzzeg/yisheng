	...
	<view class="cont">
		<scroll-view scroll-y="true" style="height: 100vh;"
			...
			<view class="blocks">
				...
				<!-- <view class="desc">
					{{ formData.snapshot.jcyy ? formData.snapshot.jcyy : formData.packageCat }}
				</view> -->
				<!--最近可约-->
			...
			<view class="blocks tabcontent" style="margin-bottom: 130upx;">
				<view class="content">
					<view class="f16 fw titles">
						<b>温馨提醒</b>
					</view>
	...
	export default {
		...
		methods: {
			...
			buttonClick(e){
				...
				uni.navigateTo({
					url: `/pages/health-examination/orderToPay?code=${this.code}&hospId=${this.hospId}`
				})
			...
			onClick(e) {
				...
				this.$api.addCommunityMessage({
				    ...
				    console.log(addRes)
				    if(addRes.status == 'OK'){
                        var community = this.$store.getters.community
                     
				        uni.navigateTo({
				            url:"/pages/index/chatRoom?chatType=1&to=" + this.communityId +"&communityId="  + this.communityId+"&title="+community.mangerName
				        })
				    }
				})
			},
